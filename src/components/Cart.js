...
export default function Cart({ cartItems, totalCost, onRemoveFromCart }) {
  return (
    <div className="min-h-screen bg-gray-100 pt-20">
      ...
      {cartItems.length === 0 ? (
        <p className="text-center py-4">Cart Empty</p>
      ) : (
        <>
          <div className="mx-auto max-w-5xl justify-center px-6 md:flex md:space-x-6 xl:px-0">
            <div className="rounded-lg md:w-2/3">
              {cartItems.map((item) => (
                <div
                  key={item.id}
                  className="justify-between mb-6 relative rounded-lg bg-white p-6 shadow-md sm:flex sm:justify-start"
                >
                  <img
                    src={item.image}
                    alt="product-image"
                  ...
                  <div className="sm:ml-4 sm:flex sm:w-full sm:justify-between">
                    <div className="mt-5 sm:mt-0">
                      ...
                      <p className="mt-1 mb-0 text-xs text-gray-700">
                        {item.price}
                      </p>
                      <p className="text-sm">
                        {" "}
